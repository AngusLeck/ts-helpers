 ...
 */
type Depth<N extends number, Acc extends 0[] = []> = Acc["length"] extends N
  ? Acc
  : Depth<N, [...Acc, 0]>;
 ...
 */
type DecrementDepth<D extends unknown[]> = D extends [unknown, ...infer Rest]
  ? Rest
  : [];
 ...
 */
type IsDepthExhausted<D extends unknown[]> = D extends [] ? true : false;
